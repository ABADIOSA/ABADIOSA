    ...
    if is_live {
        ...
        if let Ok(base) = app.path().app_cache_dir() {
            ...
            if let Some(s) = dvr.to_str() {
                // Must be "demuxer-cache-dir" (demux/cache.c): mpv exposes no "cache-dir"
                // property, so the old name was silently rejected. cache-on-disk then fell
                // back to mpv's config dir, which embedded libmpv does not have, and every
                // segment failed with "Failed to create file cache".
                if let Err(e) = mpv.set_property("demuxer-cache-dir", s) {
                    eprintln!("[harbor::mpv] demuxer-cache-dir rejected: {:?}", e);
                }
            }
        }
