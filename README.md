...
هذه نسخة شخصية مخصصة من مشروع [Harbor](https://github.com/harborstremio/harbor) مفتوح المصدر — عميل سطح مكتب لمنظومة Stremio مبني بـ Tauri 2 وReact وRust مع مشغّل libmpv أصلي. المشروع الأصلي مرخّص برخصة MIT التي تسمح بالنسخ والتعديل، وكل الحقوق والشكر لفريق Harbor الأصلي.

## ما الذي تغيّر في هذه النسخة؟

- **ترجمة عربية كاملة 100%**: اكتملت ترجمة كل نصوص الواجهة (4636 نصًا) بعد أن كانت ناقصة 909 نصوص في المشروع الأصلي، مع الحفاظ على كل متغيرات النصوص البرمجية. الواجهة تدعم الاتجاه من اليمين لليسار (RTL) تلقائيًا عند اختيار العربية.
- **العربية لغة احتياطية**: إذا لم تُطابق لغة النظام أي لغة مدعومة، تظهر الواجهة بالعربية بدلًا من الإنجليزية.
- **تعطيل التحديث التلقائي من الخادم الرسمي**: حتى لا يستبدل تحديثٌ من harbor.site هذه النسخة المخصصة. التحديث يتم يدويًا بسحب آخر تغييرات المصدر وإعادة البناء.
- **هوية مخصصة**: عنوان النافذة أصبح «Harbor • Abadiosa Edition».
...
pnpm run setup      # جلب mpv و ffmpeg والخطوط
pnpm tauri dev      # التشغيل للتطوير
pnpm tauri build    # بناء مثبّت لنظامك
```

<div dir="rtl" align="right">

أو استخدم ورك فلو **Tauri Build** من تبويب Actions في GitHub لبناء مثبّتات ويندوز وماك ولينكس جاهزة دون بيئة تطوير محلية.

> **تنبيه**: هذا التطبيق مشغّل وواجهة فقط — لا يستضيف أي محتوى ولا يأتي بأي إضافات محتوى. أنت من يضيف مصادره، وعليك الالتزام بأنظمة بلدك. راجع إخلاء المسؤولية في الأسفل.
