...
// Abadiosa edition: upstream auto-updates would replace this customized build,
// so update checks against harbor.site are disabled.
...
export async function checkForUpdate(manual = false): Promise<void> {
  if (!IS_TAURI) return;
  if (UPDATES_DISABLED) {
    if (manual) set({ status: "uptodate", manualCheck: true, version: null, notes: null });
    return;
  }
...
export function startUpdateWatcher(): void {
  if (started || !IS_TAURI || UPDATES_DISABLED) return;
  started = true;
